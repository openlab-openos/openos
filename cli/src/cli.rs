...
pub enum CliError {
    ...
    #[error("Account {1} has insufficient funds for fee ({0} BTG)")]
    InsufficientFundsForFee(f64, Pubkey),
    #[error("Account {1} has insufficient funds for spend ({0} BTG)")]
    InsufficientFundsForSpend(f64, Pubkey),
    #[error("Account {2} has insufficient funds for spend ({0} BTG) + fee ({1} BTG)")]
...
pub fn parse_command(
    ...
    let response = match matches.subcommand() {
        ...
        ("completion", Some(matches)) => {
            ...
            .gen_completions_to("openos", shell_choice, &mut stdout());
            std::process::exit(0);
        }
        // Cluster Query Commands
...
mod tests {
    ...
    fn test_cli_process_command() {
        ...
        assert_eq!(process_command(&config).unwrap(), "0.00000005 BTG");
