...
case $URL in
stable)
  URL=http://api.devnet.openverse.network
  ;;
localhost)
...
case $TAG in
edge|beta)
  DOWNLOAD_URL=https://release.openverse.network/"$TAG"/solana-release-$TARGET.tar.bz2
  ;;
*)
  DOWNLOAD_URL=https://github.com/openverse-labs/openverse/releases/download/"$TAG"/openverse-release-$TARGET.tar.bz2
  ;;
esac
